...
class enemy2{
    ...
    update()
    {
        ...
        else if(this.position.x+2000<=1000)
        { this.velocity.x =5}
...
function animate()
{
    ...
    c.save()    
    c.fillStyle="red"    
    c.fillRect(50,canvas.height-100,40,-hp)
    c.save()
    c.restore()
    c.strokeStyle="yellow"
    c.strokeRect(50,canvas.height-100,40,-200)
    console.log(random)
    c.font = "60px Georgia"
    c.fillText("Màn:"+wase,50,50)
    c.fillText("Đạn:"+lvdan,50,150)
    if(energy>0)
    {
        energy--
    }
    c.save()    
    c.fillStyle="yellow"    
    c.fillRect(canvas.width-100,canvas.height-100,40,-energy*10)
    c.strokeStyle="yellow"
    c.strokeRect(canvas.width-100,canvas.height-100,40,-1000)
